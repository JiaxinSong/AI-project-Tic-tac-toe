...
def exminimax(board, d, rule, action, depth, node, l,data):  # action = (x,y)   data=(board,score)
    l.append(1)
    e, f = action
    ...
    if end == d:  # d win
        ...
        sco = score(matrix, rule)
        data.append((matrix, -sco))
        # print(data)
        ...
        if depth == 0:
            ...
            for i in ca:
                ...
                child_utilities1.append(exminimax(board, b, rule, i, depth - 1,node, l,data))
                recover_act(board, b, x, y)


                make_act(board, a, x, y)
                child_utilities2.append(exminimax(board, a, rule, i, depth - 1, node,l,data))
...
def AI_action(board, d, rule, depth,node,data):  # return ((score,position),nodes)
    ...
    for i in valid_actions1(board):
        ...
        nscore = exminimax(newboard, d, rule, i, depth - 1, node,l,data)
        # print(data)
        res.append((nscore, i))
